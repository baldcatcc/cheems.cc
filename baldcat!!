                ...
                function bindable.OnInvoke(response)
                    if response == "Yes" then
                        ...
                        StarterGui:SetCore(
                            ...
                            {
                                Title = "cheem.cc",
                                Text = "go check workspace you tard",
                                Duration = 0.0000001
                            }
                        )
                    end
...
do
	...
	local IMGUIWindow = library:AddWindow("cheem.cc", {
	...
	local rageBot = IMGUIWindow:AddTab("br tab")
	local silentAimbotTab = IMGUIWindow:AddTab("silant aim")
	local aimlockTab = IMGUIWindow:AddTab("kimbladi")
	local espTab = IMGUIWindow:AddTab(".")
	local miscTab = IMGUIWindow:AddTab(".")
	local anotherTab = IMGUIWindow:AddTab(".")
	...
	do
		...
		local silenabled = silentAimbotTab:AddSwitch("no need for seetings silent aim",function(state)

			getgenv().SilentAimbot = state
		
		end)

		local silenabled2 = silentAimbotTab:AddSwitch("hit airshots but laggy",function(state)

			getgenv().AirshotSilent = state
		
		end)

		local silenabled2 = silentAimbotTab:AddSwitch("webhok",function(state)

			getgenv().Webhookmode = state
		
		end)

		local silenabled3 = silentAimbotTab:AddSwitch("tells you who you lock to",function(state)

			getgenv().NotifMode = state
		
		end)

		local silenabled4 = silentAimbotTab:AddSwitch("copy username to clipboard",function(state)

			getgenv().CopyMode = state
		
		end)

		silentAimbotTab:AddLabel("cool things")

        
		anotherTab:AddLabel("credats")
		anotherTab:AddLabel("me")
		anotherTab:AddLabel("my brains")
		anotherTab:AddLabel("my hands")
		anotherTab:AddLabel("my eyes")

		miscTab:AddLabel("If you die press fix")
		miscTab:AddLabel("basically real macro written lua")
	    ...
	    local aimlockEnable = aimlockTab:AddSwitch("kimbladi", function(state)
            getgenv().Aimlock = not getgenv().Aimlock
        end)

        local aimlockEnable = aimlockTab:AddSwitch("hit all airshot but can be glitchy", function(state)
        ...
        aimlockTab:AddTextBox("aim length", function(a)
		...
		rageBot:AddLabel("beta tings")

		local triggerbotEnable = rageBot:AddSwitch("no work 85% time dont use", function(state)
		...
		rageBot:AddTextBox("spinny spin", function(a)
		...
		rageBot:AddTextBox("funni lag", function(a)
		...
		local espEnable = espTab:AddSwitch("know where people are", function(onoff)
		...
		local tracersEnable = espTab:AddSwitch("tracer", function(onoff)
		...
		local namesEnable = espTab:AddSwitch("shows you esp names", function(onoff)
		...
		local boxesEnable = espTab:AddSwitch("boxes around players", function(onoff)
		...
		miscTab:AddButton("press to fix antilock", function()
		...
		miscTab:AddTextBox("antilock (use -0.10 -0.60 can be used for speed glitch as well)", function(a)
		...
		miscTab:AddButton("antilock best (Z)", function()
		...
		miscTab:AddButton("speed glitch (x)", function()
			local Player = game:GetService("Players").LocalPlayer
			local Mouse = Player:GetMouse()
		...
		bodySelect:Add("head")
		bodySelect:Add("uptorso")
		bodySelect:Add("humanoidroot")
		bodySelect:Add("lowtorso")
		...
		bodySelect2:Add("uptorso")
		bodySelect2:Add("humanoidroot")
		bodySelect2:Add("lowtorso")
